...
namespace Ramune.SeaglideUpgradesModules.Monos
{
    public class SeaglideUpgradeManager : MonoBehaviour
    {
        ...
        [
            Prefabs.UpgradeModules.BatterySwapUpgrade.Prefab.Info.TechType,
            Prefabs.UpgradeModules.BoostUpgrade.Prefab.Info.TechType,
            Prefabs.UpgradeModules.EfficiencyUpgrade.Prefab.Info.TechType,
            Prefabs.UpgradeModules.LightUpgrade.Prefab.Info.TechType,
            Prefabs.UpgradeModules.NoiseDampeningUpgrade.Prefab.Info.TechType,
            Prefabs.UpgradeModules.PowerglideUpgrade.Prefab.Info.TechType,
            Prefabs.UpgradeModules.SpeedUpgrade.Prefab.Info.TechType,
        ];
        ...
        public void ApplyCurrentLights()
        {
            switch(techType)
            {
                case var x when x == SeaglideUpgrades.Prefabs.Deployables.SeaglideMK1.Prefab.Info.TechType:
                    SeaglideUpgrades.Config.OnChangeMK1();
                    break;

                case var x when x == SeaglideUpgrades.Prefabs.Deployables.SeaglideMK2.Prefab.Info.TechType:
                    SeaglideUpgrades.Config.OnChangeMK2();
                    break;

                case var x when x == SeaglideUpgrades.Prefabs.Deployables.SeaglideMK3.Prefab.Info.TechType:
        ...
        public void RebuildModuleCache()
        {
            ...
            foreach(var techType in moduleAllowedTech)
            {
                ...
                foreach(var item in items)
                {
                    ...
                    {
                        case var x when x == Prefabs.UpgradeModules.BatterySwapUpgrade.Prefab.Info.TechType:
                            batterySwapModules++;
                            break;

                        case var x when x == Prefabs.UpgradeModules.BoostUpgrade.Prefab.Info.TechType:
                            boostModules++;
                            break;
                        ...
                        case var x when x == Prefabs.UpgradeModules.NoiseDampeningUpgrade.Prefab.Info.TechType:
                            noiseModules++;
                            break;
        ...
        public bool CanAddModuleItem(Pickupable pickupable, bool verbose)
        {
            ...
            {
                var x when x == Prefabs.UpgradeModules.BatterySwapUpgrade.Prefab.Info.TechType =>
                    batterySwapModules < SeaglideUpgradesModules.config.GetMaxBatterySwapUpgrades(),

                var x when x == Prefabs.UpgradeModules.BoostUpgrade.Prefab.Info.TechType =>
                    boostModules < SeaglideUpgradesModules.config.GetMaxBoostUpgrades(),

                var x when x == Prefabs.UpgradeModules.EfficiencyUpgrade.Prefab.Info.TechType =>
                    efficiencyModules < SeaglideUpgradesModules.config.GetMaxEfficiencyUpgrades(),

                var x when x == Prefabs.UpgradeModules.LightUpgrade.Prefab.Info.TechType =>
                    lightModules < SeaglideUpgradesModules.config.GetMaxLightUpgrades(),

                var x when x == Prefabs.UpgradeModules.NoiseDampeningUpgrade.Prefab.Info.TechType =>
                    noiseModules < SeaglideUpgradesModules.config.GetMaxNoiseDampeningUpgrades(),

                var x when x == Prefabs.UpgradeModules.PowerglideUpgrade.Prefab.Info.TechType =>
                    powerglideModules < SeaglideUpgradesModules.config.GetMaxPowerglideUpgrades(),

                var x when x == Prefabs.UpgradeModules.SpeedUpgrade.Prefab.Info.TechType =>
