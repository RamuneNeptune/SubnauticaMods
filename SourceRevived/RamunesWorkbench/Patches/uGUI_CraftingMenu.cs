...
namespace Ramune.RamunesWorkbench.Patches
{
    ...
    public static class uGUI_CraftingMenuPatches
    {
        ...
        {
            if(CurrentCraftTreeType != Prefabs.Buildables.RamunesWorkbench.craftTreeType || node == null ||  node.icon == null || node.action != TreeAction.Expand)
                return;
        ...
        {
            if(CurrentCraftTreeType != Prefabs.Buildables.RamunesWorkbench.craftTreeType || __instance.GetNode(icon) is not uGUI_CraftingMenu.Node node || node == null || node.action != TreeAction.Expand)
                return;
